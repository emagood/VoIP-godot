...
@onready var spec = AudioServer.get_bus_effect_instance(0,0)
	# esto en la delaraccion principal
...
func _input(event: InputEvent) -> void:
	...
	var sec_inSound = musica.get_playback_position() # posicion del audio actual reproduciendo 
	if Input.is_key_pressed(KEY_A): prints(musica.has_stream_playback() ) #si existen datros en el puntero
	if Input.is_key_pressed(KEY_B): prints(musica.is_playing()) # si la musica se esta reproduciendo
	if Input.is_key_pressed(KEY_P): prints(sec_inSound)  # para mostrar el tiempo transcurido
	if Input.is_key_pressed(KEY_S): musica.seek(100) # posicion de musica que quiero 
	
	if musica.has_stream_playback() == null:  # me fijo si tengo algunos datos cargados
		prints("no se cargo el audio o musica")
		return
	if musica.autoplay ==  true: # si el reproductor repetir esta activado
		musica.autoplay =  false # lo desactivo si no lo quiero
	
	if musica.is_playing() == false: # si no se reprodue entonces 
		#musica.play()           # lo pongo en marcha
		musica.seek(100)       # en este lugar de pista
	
	if sec_inSound >= 180:    # si la pista pasa mas de tanto
		musica.seek(100)      # lo llevo a lugar asiendo un bucle
	pass
